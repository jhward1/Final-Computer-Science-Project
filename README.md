...
To persist data files across sessions to a GitHub repository, add these to your `.env`:
...
2. Select a judge model from the dropdown. Available options include:
   - **Fine-Tuned Judge (Tinker)** — a model fine-tuned specifically for this classification task (requires Anthropic internal Tinker access)
...
### Step 3 — Analysis Dashboard (Tab 3)
...
Click any row in the filtered results table to open a detailed view showing the original prompt, the model's answer, and the judge's analysis with color-highlighted evidence spans (yellow = certainty markers, blue = framework markers).

**Model Comparison** — Select a question and up to three models to see their answers and judge classifications side by side.

### Step 4 — Model Config (Tab 4)

Browse the full OpenRouter model catalog, filter by provider and release date, and save your selection to `models_config.json`. Non-OpenRouter entries (Groq, Google) already in the config are always preserved. Changes here take effect immediately in Tab 1.
